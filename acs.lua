...
MainTab:NewToggle("Silent Aim", function(state)
    Settings.Aimbot.SilentAimUsed = state
end)
    ...
    Run = zzRunService.RenderStepped:Connect(function()
        if plr.Character ~= nil and plr.Character:FindFirstChild("HumanoidRootPart") and zzLPlayer ~= plr and plr then
           ...
           if onScreen then
            ...
            if (Settings.Aimbot.AimbotUsed or Settings.Aimbot.SilentAimUsed) and plr == AimPlayer then
                Dist.Color = Color3.new(1,1,1)
                Name.Color = Color3.new(1,1,1)
                Box.Color = Color3.new(1,1,1)
                Tracer.Color = Color3.new(1,1,1)
...
local Params = {"Instance", "Vector3", "Vector3", "RaycastParams"}
...
oldNamecall = hookmetamethod(game, "__namecall", newcclosure(function(...)
    ...
    local Args = {...}
    local self = Args[1]
    if Settings.Aimbot.SilentAimUsed and self == workspace and not checkcaller() and AimPlayer ~= nil and math.random(1, 100) <= Settings.Aimbot.HitChance then
        if Method == "Raycast" then
            if ValidateArgs(Args) then
               -- print(tostring(Script))
                local Origin = Args[2]
                local HitPart = AimPlayer.Character[GetPart(AimPlayer.Character)]
                if HitPart then
